...
import com.treblemaker.generator.decoder.ChordMelodyDecoder;
...
public class MelodyLSTM {
    ...
    public MelodyLSTM(Config config){
        this.config = config;
    }
    ...
    public void run(String keyAndChords) throws Exception {
        ...
        //Save the model
        File savedLocation = new File(Paths.get("trained-melody-model.zip").toString());      //Where to save the network. Note: the file is in .zip format - can be opened externally
        ...
        if (restored != null) {
            ...
            for (int j = 0; j < samples.length; j++) {
                ...
                String validString = decoder.extractValidMelody(samples[j]);
                ...
                if (validString.equalsIgnoreCase("invalid")) {
                    ...
                    try {
                        ...
                        MidiFileManager.savePatternToMidi((PatternProducer) pattern, new File(Paths.get("output/" + fileName).toString()));
